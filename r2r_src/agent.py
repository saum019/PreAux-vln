...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        self.decoder = model.AttnDecoderLSTM(args.aemb, args.rnn_dim, args.dropout,
        ...
        if args.aux_option:
            if args.modspe:
                ...
                self.speaker_decoder = model.SpeakerDecoder(self.tok.vocab_size(), args.wemb,
            ...
            self.aux_models = (
            self.speaker_decoder, self.progress_indicator, self.matching_network, self.feature_predictor,
           )
    ...
    def _feature_variable(self, obs):
        ...
        if args.sparseObj and not (args.denseObj):
            ...
            return Variable(torch.from_numpy(sparseObj), requires_grad=False).cuda(), Variable(
                torch.from_numpy(denseObj), requires_grad=False).cuda(), \
        ...
        for t in range(self.episode_len):
            ...
            if args.aux_option:
                if args.modfea:
                    ...
                    if args.mask_fea:
                        fea_loss += torch.mean(
                            F.mse_loss(feature_pred, feature_label, reduce=False) * (~_mask).float().unsqueeze(1))
                        #ang_loss += torch.mean(
                         #   F.mse_loss(angle_pred, angle_label, reduce=False) * (~_mask).float().unsqueeze(1))
                    else:
                ...
                if args.modmat:
                    for i in range(v_ctx.shape[1]):
                        if i == 0:
                            ...
                            h1 = h1 * (1 - valid_mask.float().unsqueeze(1)) + _h1 * valid_mask.float().unsqueeze(
                                1)  # update active feature
    ...
    def _dijkstra(self):
        ...
        for i, idx in enumerate(perm_idx):
            recover_idx[idx] = i
        ctx, h_t, c_t = self.encoder(seq, seq_lengths)
        ctx, h_t, c_t, ctx_mask = ctx[recover_idx], h_t[recover_idx], c_t[recover_idx], seq_mask[
            recover_idx]  # Recover the original order
        ...
        for _ in range(300):
            ...
            for i, (state_id, state) in enumerate(smallest_idXstate):
                ...
                self.env.env.sims[i].newEpisode(scan, next_viewpoint, heading,
    ...
    def beam_search(self, speaker):
        ...
        for result in results:
            ...
            seq_lengths = np.argmax(insts == self.tok.word_to_index['<EOS>'],
                                    axis=1)  # len(seq + 'BOS') == len(seq + 'EOS')
