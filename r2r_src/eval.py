...
class Evaluation(object):
    ...
    def _score_item(self, instr_id, path,iter, env_name):
        ...
        #print("instr_id", instr_id, "start",start, "goal", goal, "final_pos", final_position, "no of steps",len(path)-1, "nav_error", self.distances[gt['scan']][final_position][goal]  )
        self.values["instr_id"]=instr_id
        self.values["start"]=start
        self.values["goal"]=goal
        self.values["final_pos"]=final_position
        self.values["no of steps"]= len(path) - 1
        self.values["nav_error"]=self.distances[gt['scan']][final_position][goal]
        self.values["trajectory"]=path        
        #print("PATH", path, "GT", gt)
        ...
        self.values['trajectory_length'] = distance
        self.scores['shortest_lengths'].append(
            self.distances[gt['scan']][start][goal]
        )
        self.values['shortest_length']= self.distances[gt['scan']][start][goal]
        
        #Saving the resulting paths and values
        fname="%s.txt" %iter
        with open("output/data/"+env_name+"/" +fname, "a+") as fi:
            fi.write(json.dumps(self.values))
            fi.write(",")
            #print(decoded_file)
        


        #with open("output/values/"+fname, "a+") as fi:
            #json.dump(data, fi)



    def score(self, output_file, iter, env_name):
        ''' Evaluate each agent trajectory based on how close it got to the goal location '''
        self.scores = defaultdict(list)
        self.values={}
        #print(iter)
        fname="%s.txt" %iter
        with open("output/data/"+env_name+"/"+fname, "w") as f:
            f.write("[")
        ...
        #Saving the resulting paths
        r=json.dumps(results, indent=4)
        print("iter", iter)
        fname="%s.json" %iter
        f=open("output/trajectories/"+env_name+"/"+fname, "w")
        f.write(r)
        f.close()
        ...
        for item in results:
            ...
            if item['instr_id'] in instr_ids:
                ...
                self._score_item(item['instr_id'], item['trajectory'],iter, env_name)
        
        # Add closing square bracket at the end.
        fname="%s.txt" %iter
        with open("output/data/"+env_name+"/" +fname, "a+") as f:
