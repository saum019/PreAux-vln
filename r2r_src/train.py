...
def train_speaker(train_env, tok, n_iters, log_every=500, val_envs={}):
    ...
    for idx in range(0, n_iters, log_every):
        ...
        avg=speaker.train(interval,idx)   # Train interval iters
        l[idx+interval]=avg.item()
        e[idx+interval]={}
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            e[idx+interval][env_name]={"Bleu":tuple(precisions),"word_accu": word_accu, "loss":loss,  "bleu_score":bleu_score}
            #print( e[idx+interval][env_name])
        print(('%s (%d %d%%) %s' % (timeSince(start, float(idx)/n_iters), iter, float(idx)/n_iters*100)))
    with open("tasks/R2R/data/speaker_data_final.json", "a+") as fi:
        fi.write(json.dumps(e, indent=3))
    with open("tasks/R2R/data/loss_speaker_final_avg.json", "a+") as fi:
        fi.write(json.dumps(l, indent=3))
...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    if args.fast_train:
        log_every = 40
    with open("output/train_score/train.csv", 'w') as f:
        f.write("idx,total,rl_loss,ml_loss,critic_loss,spe_loss,pro_loss,mat_loss,fea_loss,entropy,predict_loss")
        f.write("\n")
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        #print("TRAIN SCORE IS", total,rl_loss,ml_loss,critic_loss,spe_loss,pro_loss,mat_loss,fea_loss,entropy,predict_loss)
        #exit()
        with open("output/train_score/train.csv", 'a') as f:
            f.write('%d, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f' % (idx,total,rl_loss.item(),ml_loss.item(),critic_loss,spe_loss.item(),pro_loss.item(),mat_loss.item(),fea_loss.item(),entropy,predict_loss))
            f.write("\n")
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            score_summary, _ = evaluator.score(result, iter, env_name)
            loss_str += "%s " % env_name

            f_name="%s.csv" % env_name
            #print("env_name:", f_name)
           # print(score_summary)

            with open("output/score/" + f_name, 'a') as f:
                f.write('%d'%(idx))
                for key in score_summary.keys():
                    f.write("%.4f,"%(score_summary[key]))
                f.write("\n")
            #print(score_summary)
...
def train_val():
    ...
    if args.denseObj:
        #print(dense)
        print(np.load(dense_obj_feat1, allow_pickle=True).item())
        print(dense_obj_feat1)
        print(dense_obj_feat2)
        obj_d_feat = utils.read_obj_dense_features(dense_obj_feat1, dense_obj_feat2, bbox, sparse_obj_feat, args.objthr)

    featurized_scans = set([key.split("_")[0] for key in list(feat_dict.keys())])

    train_env = R2RBatch(feat_dict, obj_d_feat=None, obj_s_feat=None, batch_size=args.batchSize,
    ...
    val_envs = OrderedDict(
        ((split,
          (R2RBatch(feat_dict, obj_d_feat=None, obj_s_feat=None, batch_size=args.batchSize, splits=[split],
                    tokenizer=tok),
