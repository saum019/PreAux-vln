...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx, 
        ...
        glove_vec = json.load(open("tasks/R2R/data/trainval_glove.json", 'r', encoding="utf-8"))
        embed = torch.nn.Embedding(vocab_size, embedding_size, padding_idx)
        self.embedding = embed.from_pretrained(torch.FloatTensor([list(map(float, g)) for g in glove_vec.values()]))
